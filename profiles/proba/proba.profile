...
define('proba_EDITOR', 'ckeditor');

// Define the allowed filtered html tags
define('proba_FILTERED_HTML', '<a> <img> <br> <em> <p> <strong> <cite> <sub> <sup> <span> <blockquote> <code> <ul> <ol> <li> <dl> <dt> <dd> <pre> <address> <h2> <h3> <h4> <h5> <h6>');

// Define the "manager" role name
define('proba_MANAGER_ROLE', 'manager');

// Define the default theme
define('proba_THEME', 'cube');

// Define admin theme
define('proba_THEME_ADMIN', 'rubik');


// Define the default frontpage
define('proba_FRONTPAGE', 'node/1');
 ...
 */
function proba_profile_modules() {
  $modules = array(
   ...
   // 'ctools', 'context', 'context_ui',
    ...
    // Misc
    'admin_menu', 'auto_nodetitle', 'backup_migrate', 'formfilter', 'imce', 'imce_wysiwyg', 'jquery_update', 'login_destination', 'menutrails', 'jquery_ui', 'path_redirect', 'semanticviews', 'views_bulk_operations', 'diff', 'pathauto', 'vertical_tabs', 'devel', 'module_filter', 'token',
 ...
 */
function proba_profile_details() {
  
  return array(
    'name' => 'proba Website',
    'description' => 'proba Website.'
  );
}
 ...
 *   task list.
 */
function proba_profile_task_list() {
  global $conf;
  $conf['site_name'] = 'proba Website';
 ...
 */
function proba_profile_tasks(&$task, $url) {

  // Include the enabled modules
  install_include(proba_profile_modules());
  
  // Use the file to make a node
  install_node_export_import_from_file('profiles/proba/content/nodes.import.inc', array('status'=>1), $user, FALSE);

  proba_build_directories();
  proba_config_menu();
  proba_config_roles();
  proba_config_perms();
  proba_config_filter();
  proba_config_wysiwyg();
  proba_config_theme();
  proba_config_vars();
  proba_cleanup();
  ...
  variable_set('site_footer', st('&copy; Copyright 2013 by Proba. Proba Website.  All rights reserved.'));
 ...
 */
function proba_build_directories() {
 ...
 * Configure wysiwyg
 */
function proba_config_wysiwyg() {
  // Load external file containing editor settings
  include_once('proba_editor.inc'); 
  
  // Add settings for 'Filtered HTML'
  $item = new stdClass;
  $item->format = 1;
  $item->editor = proba_EDITOR;
  $item->settings = serialize(proba_editor_settings('Filtered HTML'));
  ...
  $item->format = 2;
  $item->editor = proba_EDITOR;
  $item->settings = serialize(proba_editor_settings('Full HTML'));
  drupal_write_record('wysiwyg', $item);
}
...
function proba_config_roles() {
  ...
  db_query("INSERT INTO {role} (rid, name) VALUES (3, '%s')", t(proba_MANAGER_ROLE));
 ...
 */
function proba_config_perms() {
  // Load external permissions file
  include_once('proba_perms.inc');
  ...
  $permissions = proba_import_permissions();
 ...
 * Configure theme
 */
function proba_config_theme() {
  // Enable garland
  db_query("UPDATE {system} SET status = 0 WHERE type = 'theme' and name = '%s'", 'garland');
  
  // Enable Basic
  db_query("UPDATE {system} SET status = 1 WHERE type = 'theme' and name = '%s'", proba_THEME_ADMIN);
  
  // Enable Basic theme
  db_query("UPDATE {system} SET status = 1 WHERE type = 'theme' and name = '%s'", proba_THEME);
  
  // Set Basic theme as the default
  variable_set('admin_theme', proba_THEME_ADMIN);
  
  // Set Basic theme as the default
  variable_set('theme_default', proba_THEME);
 ...
 */
function proba_config_vars() {
  // Set default homepage
  variable_set('site_frontpage', proba_FRONTPAGE);
...
function proba_cleanup() {
  ...
  watchdog('proba', t('Welcome to Proba!'));
}
